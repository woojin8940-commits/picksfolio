...
export interface MetaLink {
   ...
   *
   * 연동은 세 화면이 함께 쓰는 하나지만, 해제는 누른 화면의 몫만 끈다. 자동 디엠
   * 해제는 인사이트까지 함께, 브랜드 매칭받기 해제는 그 하나만이다
   * (`disconnectLinkFeature`). 그래서 토큰을 지우는 대신 어느 기능이 꺼졌는지만
   * 여기 적어 두고, 읽는 쪽이 자기 기능을 확인한다
   * (`resolveSharedLink` 의 두 번째 인자).
   *
   * 반대로 연동은 기능을 가리지 않는다. 어느 화면에서 계정을 다시 붙이든 이 목록은
   * 통째로 지워지고 세 기능이 함께 살아난다 — 사람이 하는 일은 "이 계정을 붙인다"
   * 하나인데 화면마다 붙는 범위가 다르면, 붙였는데도 비어 있는 메뉴가 남는다.
   */
  featuresOff?: string[];
}
 ...
 *   collab   — 브랜드 매칭받기(브랜드가 보는 명단의 확인된 숫자, 협업 캠페인).
 *   insights — 인사이트(본인만 보는 릴스 성과 화면).
 *
 * 인사이트에는 자기 해제 버튼이 없다. 그래도 목록에 있는 이유는 자동 디엠을 끊으면
 * 함께 꺼지는 쪽이기 때문이다(`disconnectLinkFeature`) — 자동 디엠 화면의 해제는
 * 사람에게 "내 인스타그램을 픽스폴리오가 읽는 일을 그만둔다"는 뜻이고, 그 화면에서
 * 끊었는데 같은 계정의 릴스 성과가 다른 메뉴에 계속 떠 있으면 끊은 것이 무엇인지
 * 알 수 없어진다.
 *
 * 브랜드 매칭받기 해제는 인사이트를 건드리지 않는다. 그쪽은 "브랜드에게 내 숫자를
 * 보내지 않겠다"는 뜻이라, 본인만 보는 화면을 함께 끌 이유가 없다.
 */
export type LinkFeature = "dm" | "collab" | "insights";
 ...
 * 없는 것으로 답한다. 해제는 누른 화면의 몫만 끄기 때문이다 — 브랜드 매칭받기를
 * 끊은 사람의 토큰은 그대로 남아 자동 디엠과 인사이트를 계속 받쳐 주지만, 브랜드가
 * 보는 명단에서는 "연동 안 됨"으로 다뤄야 한다. 기능을 밝히지 않으면 해제 표시와
 * 무관하게 연동을 그대로 돌려준다 — 화면이 아닌 자리(정리 작업 등)에서만 쓴다.
 *
