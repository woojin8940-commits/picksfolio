...
export interface DmContact {
   ...
   *
   * 지워진 "자주 묻는 질문" 버튼 클릭(postback)만으로 만들어진 예전 기록에 붙어
   * 있다. 버튼 클릭도 24시간 창을 열어주므로 명단에는 있어야 했지만, 그것을 "처음
   * 대화"로 소진해 버리면 이 사람이 실제로 첫 메시지를 보낼 때 인사말이 나가지
   * 않는다. 그래서 첫 메시지가 도착하면 그때를 처음으로 보고 이 표시를 지운다 —
   * 새로 붙는 일은 없지만 남아 있는 기록이 있으므로 그대로 둔다.
   */
...
export async function noteDmContact(args: {
   ...
   *  `postback`      — 버튼 클릭. 이 값을 넘기던 "자주 묻는 질문" 기능은 지웠고,
   *                    남아 있는 예전 기록을 읽기 위해서만 유지한다.
   */
