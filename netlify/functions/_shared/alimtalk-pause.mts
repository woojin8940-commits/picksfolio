 ...
 * 알림톡 발송 일시 중지 스위치.
 *
 * 솔라피에 알림톡 템플릿을 새로 올려 심사를 받는 동안에는 발송을 멈춰야 한다.
 * 심사 중인 템플릿으로 나간 발송은 거절되고, 거절된 발송은 SMS 대체 발송으로
 * 떨어져 알림톡 문구가 그대로 문자로 나간다 — 즉 "알림톡을 껐는데 문자 요금이 더
 * 나오는" 모양이 된다. 그래서 이 스위치는 채널 하나가 아니라 발송 자체를
 * 막는다(알림톡 + 그 대체 문자).
 *
 * 기본값은 "발송"이다. 예전에는 값이 없을 때를 중지로 봤는데, 그 규칙은 실제로
 * 이렇게 작동했다 — 템플릿 7개가 모두 승인되고 키·발신번호·수신자 전화번호가 전부
 * 정상인 상태에서, 사이트에 환경변수 한 줄이 없다는 이유만으로 모든 알림톡이
 * 조용히 사라졌다. 발송을 건너뛴 응답은 실패가 아니라 성공이므로 로그에도 오류가
 * 남지 않아, 원인을 찾기 전까지는 "설정은 했는데 알림이 안 온다"로만 보였다.
 * 그래서 판정을 뒤집었다: 멈추려는 사람이 명시적으로 값을 넣고, 아무 설정이
 * 없으면 나간다.
 *
 * 멈추는 방법: 환경변수 `ALIMTALK_PAUSED` 에 `true`(또는 1 · on · yes · pause · paused ·
 * stop)를 넣으면 즉시 멈춘다. 다시 내보낼 때는 그 값을 지우거나 `false` 로 바꾼다.
 * 어느 쪽이든 코드 배포는 필요 없다.
 *
 * 이 스위치가 막지 않는 것:
...
/** 이 값이 들어왔을 때만 멈춘다. 그 밖의 모든 값(빈 값 포함)은 "발송"이다. */
const PAUSE_VALUES = new Set(["true", "1", "on", "yes", "y", "pause", "paused", "stop"]);
...
export const alimtalkPaused = (): boolean =>
  PAUSE_VALUES.has(String(readEnv("ALIMTALK_PAUSED") || "").trim().toLowerCase());
