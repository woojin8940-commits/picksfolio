...
const AdminCampaignListup: React.FC<AdminCampaignListupProps> = ({ token }) => {
  ...
  // 캠페인 목록은 한 번만 읽는다. 담당자는 목록에서 캠페인을 골라 명단을 만든다.
  useEffect(() => {
    (async () => {
      const res = await apiService.getAdminCampaigns(token);
      const list = (res.campaigns || []).filter((c: any) => c.status !== 'deleted');
      setCampaigns(list);
    })();
  }, [token]);
    ...
    <div className="space-y-4">
      ...
      {!campaignId ? (
        <div className="bg-white rounded-2xl border border-slate-100 p-10 text-center">
          <p className="text-sm text-slate-400 font-bold">캠페인을 먼저 선택해 주세요.</p>
        </div>
      ) : loading ? (
        <div className="bg-white rounded-2xl border border-slate-100 p-10 text-center">
