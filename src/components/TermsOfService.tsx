      ...
      <div className="container mx-auto px-4 sm:px-6 py-16 max-w-4xl">
        ...
        <section className="mb-12">
          ...
          <div className="mb-8">
            ...
            <ul className="space-y-4 text-slate-400 text-sm leading-relaxed">
              <li><span className="text-slate-300 font-bold">정기 구독료:</span> 픽스폴리오 멤버십은 세 가지 플랜으로 운영됩니다. 스탠다드 멤버십(영상 업로드 · 콘텐츠 업로드)은 월 4,900원(VAT 포함), AI 협업 멤버십(스탠다드 혜택 + 협업 AI)은 월 6,900원(VAT 포함), 프로 플랜(스탠다드 · AI 협업 혜택 + 인스타그램 디엠 자동화)은 월 18,700원(VAT 포함)이며, 회사의 정책 변경 시 사전 공지 후 조정될 수 있습니다. 표시된 모든 금액은 부가가치세가 포함된 금액입니다. 비즈니스 수신함과 협업 타임라인은 모든 이용자에게 무료로 제공됩니다.</li>
              <li><span className="text-slate-300 font-bold">라이브 커머스 멤버십(별도 구독):</span> 라이브 커머스 송출 기능은 위 멤버십 플랜에 포함되지 않는 별도 구독 상품이며, 월 13,900원(VAT 포함)을 별도로 결제해야 이용할 수 있습니다. 프로 플랜을 포함한 다른 멤버십 플랜을 구독하더라도 라이브 커머스 멤버십을 결제하지 않으면 라이브 송출을 이용할 수 없습니다. 다만 본 조항 시행 전에 커머스 멤버십(월 13,900원)을 구독한 회원은 해당 구독을 해지하기 전까지 종전 조건으로 라이브 커머스를 계속 이용할 수 있습니다.</li>
        ...
        <section className="mb-12">
          ...
          <div className="mb-8">
            ...
            <ul className="space-y-4 text-slate-400 text-sm leading-relaxed">
              ...
              <li>
                ...
                <ul className="mt-2 ml-4 space-y-2">
                  <li>결제 후 7일 이내에 라이브 방송을 단 1회도 송출하지 않았고, 비즈니스 타임라인 등 유료 기능 또한 이용하지 않은 경우에 한하여 「전자상거래 등에서의 소비자보호에 관한 법률」 제17조에 따른 청약철회로 전액 환불이 가능합니다.</li>
                  <li>라이브 방송 송출 이력이 1회 이상 있거나 유료 기능을 이용한 내역이 있는 경우, 해당 월의 구독료는 환불되지 않습니다.</li>
                  <li>카드 결제 후 취소 또는 환불 요청 시 결제된 카드로 카드매출 취소 요청되며, 카드사 처리 기준에 따라 영업일 2~5일 이내 카드결제 취소 처리됩니다.</li>
                </ul>
              </li>
              <li>
                <span className="text-slate-300 font-bold">잔여일에 대한 청약철회 및 이용 보장:</span>
                <ul className="mt-2 ml-4 space-y-2">
                  <li>멤버십 회원(셀러)이 월 구독 기간 중 청약철회 또는 해지를 신청하는 경우, 이미 결제가 완료된 해당 월의 잔여 기간(잔여일) 동안 라이브 커머스 등 멤버십 전용 기능을 종료일까지 정상적으로 계속 이용할 수 있도록 이용 권리가 보장됩니다.</li>
                  <li>정기 구독 상품은 결제된 1개월을 하나의 이용 단위로 제공하는 서비스로, 잔여일에 해당하는 이용 권리가 해당 월의 종료일까지 보장되는 대신 잔여일을 일할 계산하여 별도로 부분 환불해 드리지는 않습니다.</li>
                  <li>잔여일에 대한 환불을 포함한 전액 환불은 위 ‘구독료 환불 규정’에 따라 결제 후 7일 이내 미사용에 해당하는 청약철회의 경우에 한하여 처리됩니다.</li>
                </ul>
              </li>
            </ul>
          </div>
          ...
          <div className="mb-8">
            ...
            <ul className="space-y-4 text-slate-400 text-sm leading-relaxed">
              ...
              <li>카드 결제 건의 취소 또는 환불 요청 시 결제된 카드로 카드매출 취소 요청되며, 카드사 처리 기준에 따라 영업일 2~5일 이내 카드결제 취소 처리됩니다.</li>
            </ul>
          </div>

          <div className="mb-8">
            <h3 className="text-lg font-bold text-white mb-3">제 8조의2 (클로드 플랜 크레딧 취소 및 환불)</h3>
            <ul className="space-y-4 text-slate-400 text-sm leading-relaxed">
              <li>클로드 플랜 크레딧은 협업 타임라인 AI에서 Claude 모델을 이용하기 위한 선불 디지털 이용권이며, 현금으로 환급되거나 서비스 외부에서 양도·전환될 수 없습니다.</li>
              <li>회원은 클로드 플랜 최초 결제 또는 크레딧 자동충전 결제 후 7일 이내에 해당 결제로 지급된 유상 크레딧을 전혀 사용하지 않은 경우 청약철회를 요청할 수 있으며, 회사는 결제 금액 전액을 환불합니다.</li>
              <li>결제 후 7일 이내라도 해당 결제로 지급된 유상 크레딧을 일부라도 사용한 경우에는 환불이 불가하며, 미사용 잔액에 대한 부분 환불도 제공되지 않습니다.</li>
              <li>결제 후 7일이 경과한 경우에는 유상 크레딧 사용 여부와 관계없이 환불이 불가합니다. 다만 회사의 귀책사유로 서비스가 정상 제공되지 않았거나 관련 법령상 환불이 필요한 경우에는 법령과 회사의 합리적인 기준에 따라 처리합니다.</li>
              <li>자동충전은 회원이 언제든지 해지할 수 있으며, 해지 후에는 다음 자동충전부터 결제가 이루어지지 않습니다. 해지 전에 이미 승인된 자동충전 결제 건은 본 조의 크레딧 환불 기준에 따라 취소 또는 환불됩니다.</li>
              <li>카드 결제 후 취소 또는 환불 요청 시 결제된 카드로 카드매출 취소 요청되며, 카드사 처리 기준에 따라 영업일 2~5일 이내 카드결제 취소 처리됩니다.</li>
            </ul>
          </div>
        ...
        <section className="mb-12">
          ...
          <div className="mb-8">
            ...
            <ul className="space-y-4 text-slate-400 text-sm leading-relaxed">
              ...
              <li>회사는 결제대행사(PG) 및 간편결제 등 결제수단 제공사(카카오페이 등)가 거래 확인을 위하여 자료를 요청하는 경우, 보유하고 있는 범위 내에서 거래사실확인서, 주문·결제 내역, 배송 증빙 등 관련 자료를 회신하며, 시청자(구매자)의 청약확인·재화의 공급·청약철회·취소·반품·환불 등 거래 이행 및 소비자 분쟁 해결이 원활히 이루어지도록 결제수단 제공사와 성실히 협조합니다.</li>
            </ul>
          </div>
