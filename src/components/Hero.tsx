...
const Hero: React.FC<HeroProps> = ({ onSignup }) => {
  ...
  return (
    <section className="relative pt-20 pb-12 md:pt-48 md:pb-32 container mx-auto px-4 sm:px-6 text-center overflow-hidden">
      ...
      >
        <h1 className="text-3xl md:text-[6.5rem] font-black leading-[1.1] md:leading-[1.05] mb-6 md:mb-12 tracking-tighter text-white font-display">
          일상을 큐레이션하다.<br />
          <span className="text-gradient">단 하나의 링크, PICKS</span>
        </h1>

        <p className="text-base md:text-2xl text-slate-400 mb-8 md:mb-16 max-w-3xl mx-auto leading-relaxed font-medium">
          데이터로 픽하고, 감각적인 그리드 템플릿으로 연결하세요.
        </p>

        <div className="relative inline-flex flex-col md:flex-row items-stretch md:items-center bg-[#11141D] border border-white/5 p-3 md:p-4 rounded-[1.5rem] md:rounded-full w-full md:w-auto shadow-2xl backdrop-blur-xl mb-8 md:mb-12">
          <div className="flex items-center px-3 md:px-8 mb-2 md:mb-0">
            <span className="text-slate-500 font-bold text-base md:text-2xl tracking-tight whitespace-nowrap">picks.me/</span>
            <input
              type="text"
              placeholder="yourname"
              className="bg-transparent border-none outline-none text-white text-base md:text-2xl font-bold p-2 flex-1 min-w-0 md:flex-none md:w-56 placeholder:text-slate-700"
              value={handle}
