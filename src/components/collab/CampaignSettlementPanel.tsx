 ...
 * 예약을 만들지 않고 읽기만 한다 — 화면에서 정산을 만들 수 있게 하면 업로드 확인과
 * 지급 근거가 어긋난다.
 *
 ...
 *
 * 예전에는 브랜드도 이 화면을 함께 썼고, 브랜드에게는 같은 행을 인플루언서 이름으로
 * 나열했다. 그런데 브랜드는 인플루언서에게 각각 송금하지 않는다 — 픽스폴리오에 한 번에
 * 보내고 개별 지급과 원천징수는 픽스폴리오가 한다. 그래서 브랜드 화면은 회차로 묶어
 * 보여 주는 BrandSettlementSummary 로 갈라 두었고, 이 화면은 "내 돈이 언제 얼마
 * 들어오는가"만 답한다.
 */
...
const CampaignSettlementPanel: React.FC<CampaignSettlementPanelProps> = ({
  ...
  useEffect(() => {
    let alive = true;
    (async () => {
      const clean = String(influencerUsername || '').replace(/^biz\//, '');
      if (!clean) {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-[11px] text-slate-400 font-medium leading-relaxed">
        표시된 금액은 지급 전 금액입니다. 원천징수(3.3%)를 뺀 금액이 입금되며, 지급일은 업로드가
        확인된 달의 다음 달 말일입니다.
      </p>
    </div>
  );
