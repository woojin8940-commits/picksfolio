...
const CampaignInsightPanel: React.FC<CampaignInsightPanelProps> = ({
  ...
  <div className="space-y-4">
    <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5">
      <p className="text-sm font-black text-blue-800">광고 업로드 이후에 인사이트가 집계됩니다</p>
    ...
    <p className="text-[11px] text-slate-400 font-medium leading-relaxed">
      인사이트는 인플루언서 채널을 Meta·YouTube 등 플랫폼에 연동해 받아옵니다. 인플루언서가 직접 적은
      수치는 근거로 쓰지 않기 때문에, 연동 전에는 위 칸이 비어 있습니다.
      {budgetKrw > 0 && ` 이 캠페인의 집행 예산은 ${formatKoreanWon(budgetKrw)}이며, 조회수가 들어오면 CPV가 자동으로 계산됩니다.`}
    </p>
  </div>
);
