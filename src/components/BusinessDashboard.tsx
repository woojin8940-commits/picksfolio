...
const REJECTION_PRESETS_EN = [
  'Proposed fee is too low',
  'Schedule conflict',
  'Category mismatch',
  'Brand concept mismatch',
...
const BusinessDashboard: React.FC<BusinessDashboardProps> = ({ userName }) => {
  ...
  const handleStatusUpdate = async (proposalId: string, status: 'accepted' | 'rejected', rejectionReasonText?: string) => {
    ...
    if (success) {
      ...
      alert(isEn ? 'Failed to update status.' : '상태 업데이트에 실패했습니다.');
    }
    setUpdatingId(null);
  };
  ...
  const handleDelete = async (proposalId: string) => {
    if (!confirm(isEn ? 'Are you sure you want to delete this proposal?' : '정말 이 제안을 삭제하시겠습니까? 삭제 후 복구할 수 없습니다.')) return;
  ...
  const filteredProposals = useMemo(() => {
    let result = openProposals.filter(p => p.category === activeTab);
    ...
    return result;
  }, [openProposals, activeTab, statusFilter, sortMode]);

  const feeSummary = useMemo(() => {
    const list = statusFilter === 'all' ? statusScoped : filteredProposals;
    const total = list.reduce((sum, p) => sum + (p.fee || 0), 0);
    return { count: list.length, total };
  }, [statusFilter, statusScoped, filteredProposals]);
  ...
  }, [statusFilter, filteredProposals]);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending':
        return <span className="px-2.5 py-1 bg-amber-100 text-amber-700 text-[10px] font-black rounded-lg">{isEn ? 'Pending' : '대기중'}</span>;
      case 'accepted':
        return <span className="px-2.5 py-1 bg-green-100 text-green-700 text-[10px] font-black rounded-lg">{isEn ? 'Accepted' : '수락됨'}</span>;
      case 'rejected':
        return <span className="px-2.5 py-1 bg-red-100 text-red-700 text-[10px] font-black rounded-lg">{isEn ? 'Declined' : '거절됨'}</span>;
      case 'completed':
        return <span className="px-2.5 py-1 bg-blue-100 text-blue-700 text-[10px] font-black rounded-lg">{isEn ? 'Completed' : '완료'}</span>;
      default:
        return null;
    }
  };
  ...
  const getDaysLeft = (endDate: string) => {
    ...
    if (diff < 0) return <span className="text-red-500 font-black text-[10px]">{isEn ? 'Closed' : '마감됨'}</span>;
      ...
      >
        ...
        <div className="text-right shrink-0">
          ...
          {proposal.revenue_share != null && proposal.revenue_share > 0 && (
            <p className="text-[10px] font-bold text-slate-400">+{proposal.revenue_share}% {isEn ? 'Share' : '배분'}</p>
          )}
        ...
        <div className="px-4 md:px-5 pb-5 border-t border-slate-100 pt-4 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-100">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest">{isEn ? 'Proposed Fee' : '제안 금액'}</p>
                <p className="text-xl font-black text-blue-700 mt-0.5">{formatFee(proposal.fee)}</p>
              </div>
              {proposal.revenue_share != null && proposal.revenue_share > 0 && (
                <div className="text-right">
                  <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest">{isEn ? 'Revenue Share' : '수익 배분'}</p>
                  <p className="text-xl font-black text-blue-700 mt-0.5">{proposal.revenue_share}%</p>
                </div>
              )}
              <div className="text-right">
                <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest">{isEn ? 'Schedule' : '일정'}</p>
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">{isEn ? 'Details' : '상세 내용'}</p>
            <p className="text-sm text-slate-700 font-medium leading-relaxed whitespace-pre-wrap">{proposal.content}</p>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[9px] font-black text-slate-400 uppercase">{isEn ? 'Company' : '회사명'}</p>
              <p className="text-xs font-bold text-slate-900 mt-0.5">{proposal.company_name}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[9px] font-black text-slate-400 uppercase">{isEn ? 'Contact Person' : '담당자'}</p>
              <p className="text-xs font-bold text-slate-900 mt-0.5">{proposal.contact_person}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[9px] font-black text-slate-400 uppercase">{isEn ? 'Email' : '이메일'}</p>
          ...
          {proposal.reference_links && proposal.reference_links.length > 0 && (
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">{isEn ? 'Reference Links' : '레퍼런스 링크'}</p>
          ...
          {proposal.attachments && proposal.attachments.length > 0 && (
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">{isEn ? 'Attachments' : '첨부 파일'}</p>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {proposal.attachments.map((url, idx) => {
                  ...
                  const fileName = url.split('/').pop() || `${isEn ? 'Attachment' : '첨부파일'} ${idx + 1}`;
                  return (
          ...
          <p className="text-[9px] text-slate-300 font-bold">
            {isEn ? 'Received: ' : '접수일: '}{formatDate(proposal.created_at)}
            {proposal.updated_at && ` · ${isEn ? 'Updated: ' : '업데이트: '}${formatDate(proposal.updated_at)}`}
          </p>
          ...
          {proposal.status === 'pending' && (
            <div className="flex gap-3 pt-2">
              ...
              >
                {updatingId === proposal.id ? (isEn ? 'Processing...' : '처리 중...') : (isEn ? '✓ Accept' : '✓ 수락하기')}
              </button>
              <button
              ...
              >
                {updatingId === proposal.id ? (isEn ? 'Processing...' : '처리 중...') : (isEn ? '✗ Decline' : '✗ 거절하기')}
              </button>
            </div>
          )}
          ...
          {(proposal.status === 'pending' || proposal.status === 'accepted' || proposal.status === 'completed') && (
            ...
            >
              <span>💬</span>
              {proposal.status === 'pending'
                ? (isEn ? 'Discuss terms before accepting' : '수락 전에 조건 상의하기')
                : (isEn ? 'Chat in Timeline' : '타임라인에서 대화하기')}
            </button>
          )}

          {proposal.status === 'rejected' && proposal.rejection_reason && (
            <div className="pt-2 px-3 py-2.5 bg-red-50 rounded-xl border border-red-100">
              <p className="text-[10px] font-black text-red-400 uppercase tracking-widest mb-1">{isEn ? 'Decline Reason' : '거절 사유'}</p>
          ...
          {proposal.status !== 'pending' && (
            <div className="flex justify-end pt-2">
              ...
              >
                ...
                {deletingId === proposal.id ? (isEn ? 'Deleting...' : '삭제 중...') : (isEn ? 'Delete' : '삭제')}
              </button>
            </div>
          )}
        </div>
  ...
  return (
    <div className="p-3 md:p-14 w-full animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 md:mb-10">
        <div>
          <h2 className="text-xl md:text-3xl font-black text-slate-900">{isEn ? 'Business Inbox' : '비즈니스 수신함'}</h2>
          <p className="text-slate-400 text-xs md:text-sm font-bold mt-1">
            {isEn ? 'Manage collaboration proposals received from brands' : '브랜드로부터 받은 협업 제안을 관리합니다'}
            {pendingCount > 0 && (
              <span className="ml-2 px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full text-[10px] font-black">
                {pendingCount} {isEn ? 'New' : '새 제안'}
              </span>
            )}
          </p>
        ...
        >
          ...
          {isEn ? 'Copy Proposal Link' : '제안 링크 복사'}
        </button>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2 md:gap-4 mb-6">
        ...
        >
          <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{isEn ? 'All' : '전체'}</p>
          <p className="text-base md:text-2xl font-black text-slate-900">{totalCount}<span className="text-xs md:text-sm font-bold">{isEn ? ' items' : '건'}</span></p>
        </button>
        <button
        ...
        >
          <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{isEn ? 'Pending' : '대기중'}</p>
          <p className="text-base md:text-2xl font-black text-amber-600">{pendingCount}<span className="text-xs md:text-sm font-bold">{isEn ? ' items' : '건'}</span></p>
        </button>
        <button
        ...
        >
          <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{isEn ? 'Accepted' : '수락됨'}</p>
          <p className="text-base md:text-2xl font-black text-green-600">{acceptedCount}<span className="text-xs md:text-sm font-bold">{isEn ? ' items' : '건'}</span></p>
        </button>
        <button
        ...
        >
          <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{isEn ? 'Declined' : '거절됨'}</p>
          <p className="text-base md:text-2xl font-black text-red-500">{rejectedCount}<span className="text-xs md:text-sm font-bold">{isEn ? ' items' : '건'}</span></p>
        </button>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest">
                {statusFilter === 'pending'
                  ? (isEn ? 'Pending Total' : '대기중 총 금액')
                  : statusFilter === 'accepted'
                  ? (isEn ? 'Accepted Total' : '수락됨 총 금액')
                  : statusFilter === 'rejected'
                  ? (isEn ? 'Declined Total' : '거절됨 총 금액')
                  : (isEn ? 'Completed Total' : '완료 총 금액')}
              </p>
              <p className="text-lg md:text-xl font-black text-blue-700">{formatFee(feeSummary.total)}</p>
            </div>
          </div>
          <p className="text-sm font-bold text-blue-400">{feeSummary.count}{isEn ? ' items' : '건'}</p>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        ...
        >
          {isEn ? '📢 Ad Proposals' : '📢 광고 제안'} <span className="ml-1 opacity-70">{adCount}</span>
        </button>
        <button
        ...
        >
          {isEn ? '🛒 Commerce Proposals' : '🛒 커머스 제안'} <span className="ml-1 opacity-70">{commerceCount}</span>
        </button>
        ...
        <div className="ml-auto">
          ...
          >
            ...
            <option value="deadline">{isEn ? 'By Deadline' : '일정순 (마감 임박)'}</option>
            <option value="fee">{isEn ? 'By Fee (High to Low)' : '금액순 (높은 순)'}</option>
          </select>
        </div>
      </div>
      ...
      {loading ? (
        <div className="text-center py-20">
          ...
          <p className="text-slate-400 font-bold text-sm">{isEn ? 'Loading...' : '로딩 중...'}</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-6 md:p-12 text-center">
          ...
          <h3 className="font-black text-slate-900 text-base md:text-lg mb-2">
            {statusFilter !== 'all'
              ? (isEn ? `No ${statusFilter} proposals` : `${statusFilter === 'pending' ? '대기중인' : statusFilter === 'accepted' ? '수락된' : statusFilter === 'rejected' ? '거절된' : '완료된'} 제안이 없습니다`)
              : (isEn ? 'No proposals found' : (activeTab === '광고' ? '광고 제안이 없습니다' : '커머스 제안이 없습니다'))}
          </h3>
          <p className="text-slate-400 text-sm font-medium">
            {statusFilter !== 'all'
              ? (isEn ? 'No proposals in this category yet.' : '해당 상태의 제안이 아직 없습니다.')
              : (isEn ? 'Share your proposal link with brands to manage proposals here.' : '브랜드에게 제안 링크를 공유하면 여기서 제안을 관리할 수 있습니다.')}
          </p>
        </div>
      ) : groupedBySchedule ? (
          ...
          {groupedBySchedule.map(([monthLabel, monthProposals]) => (
            <div key={monthLabel}>
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-[10px] font-black text-slate-400">{monthProposals.length}{isEn ? ' items · ' : '건 · '}{formatFee(monthProposals.reduce((s, p) => s + p.fee, 0))}</span>
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => { setRejectingId(null); setRejectionReason(''); }}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            ...
            <textarea
              ...
              placeholder={isEn ? 'Please enter decline reason' : '거절 사유를 입력해주세요'}
            ...
            <div className="flex gap-3">
              ...
              >
                {updatingId === rejectingId ? (isEn ? 'Processing...' : '처리 중...') : (isEn ? 'Confirm Decline' : '거절 확인')}
              </button>
            </div>
          </div>
        </div>
