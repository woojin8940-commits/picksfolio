...
import { ExternalLink, Share2, Briefcase, Search, Hash } from 'lucide-react';
...
const UserPage: React.FC<UserPageProps> = ({ username }) => {
  ...
  const [socials, setSocials] = useState(() => {
    const defaultSocials = { instagram: '', youtube: '', tiktok: '', phone: '', kakao: '', naver: '', businessProposal: false };
    try {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        console.warn('[UserPage] Site data load failed:', e);
      }
    ...
    trackView(username);
      ...
      // Debounce loadData to prevent rapid re-renders on burst realtime updates
        ...
        <footer className="py-12 flex flex-col items-center space-y-6 shrink-0">
          ...
          <div className={`w-full max-w-md mx-auto pt-6 mt-2 border-t text-center space-y-1.5 ${isDark ? 'border-white/10' : 'border-slate-200'}`}>
            <p className={`text-[11px] font-bold leading-relaxed ${subTextColor}`}>
              본 페이지의 상품 거래는 통신판매중개 플랫폼 픽스폴리오(Picksfolio)를 통해 이루어집니다.
            </p>
            <p className={`text-[11px] font-bold leading-relaxed ${subTextColor}`}>
