...
const VideoCropper: React.FC<VideoCropperProps> = ({ src, onConfirm, onCancel, aspectRatio }) => {
  ...
  useEffect(() => {
    if (!videoLoaded || !naturalSize.w) return;
  ...
  }, [videoLoaded, naturalSize, calcDisplayRect, computeFixedCropSize]);
  ...
  }, [videoLoaded, naturalSize, displayRect, calcDisplayRect, computeFixedCropSize, crop]);
  ...
  const handleConfirm = useCallback(() => {
    ...
    const rangeX = displayRect.w - crop.w;
    const rangeY = displayRect.h - crop.h;
    const posX = rangeX > 0 ? ((crop.x - displayRect.x) / rangeX) * 100 : 50;
    const posY = rangeY > 0 ? ((crop.y - displayRect.y) / rangeY) * 100 : 50;
    onConfirm({
      x: Math.round(Math.max(0, Math.min(100, posX))),
      y: Math.round(Math.max(0, Math.min(100, posY))),
    });
  }, [displayRect, crop, onConfirm]);

  const handleVideoLoaded = () => {
    if (!videoRef.current) return;
    setNaturalSize({ w: videoRef.current.videoWidth, h: videoRef.current.videoHeight });
    setVideoLoaded(true);
  };
    ...
    >
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
        <video
          ...
          muted
          playsInline
          style={videoLoaded && displayRect.w > 0 ? {
            position: 'absolute',
        ...
        {videoLoaded && displayRect.w > 0 && (
          <>
            <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none' }}>
              ...
              <rect width="100%" height="100%" fill="rgba(0,0,0,0.65)" mask="url(#video-crop-mask)" />
            </svg>
