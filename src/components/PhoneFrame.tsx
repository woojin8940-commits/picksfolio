...
const SIZE_CLASS: Record<PhoneFrameSize, string> = {
  ...
  // 한다. 폰 아래에는 라벨·"실제 페이지 확인하기" 링크·저장 버튼이 함께 쌓이므로,
  // 단순히 70vh로 잡으면 그 요소들 높이만큼 컬럼이 뷰포트를 넘쳐 폰 위/아래가 잘렸다.
  // 그래서 화면 높이에서 주변 UI(여백·라벨·링크·저장 버튼)가 차지하는 공간(~13rem)을
  // 미리 빼고 남는 높이에 맞춰 폰을 키운다. 너비는 9/19.5 비율로 자동 계산되며,
  // 어떤 화면에서도 기기 전체가 잘리지 않고 중앙에 균형 있게 보인다.
  xl: 'h-[min(720px,calc(100vh_-_13rem))] w-auto',
};
