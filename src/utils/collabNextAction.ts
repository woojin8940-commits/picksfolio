    ...
    case 'upload': {
      ...
      if (!String(input.uploadUrl || '').trim()) {
        return {
          ...base,
          owner: 'influencer',
          // 영상 검토가 끝났다는 것은 "이 영상 그대로 나간다"는 확정이지 "지금 올려라"는
          // 아니다. 올리는 날은 캠페인 전체 일정과 맞물려 담당자가 잡으므로, 검토가
          // 닫히자마자 올려 버리면 같은 주에 여러 편이 겹치거나 브랜드가 정한 공개일보다
          // 먼저 나간다.
          todo: '담당자와 소통 후 업로드 일정을 정하고, 게시한 뒤 게시물 링크와 광고 파트너십 코드를 남겨 주세요.',
          short: '업로드 · 링크 등록 필요',
